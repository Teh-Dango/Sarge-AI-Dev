...
if (SAR_DEBUG) then {diag_log format["Sarge AI System: Triggered by: %1", _player];};
...
for "_i" from (count _grps_band) to ((_max_grps select 0) - 1) do
{
    if(_max_p_grp select 0 > 0) then {
        ...
        if(_chance < _probability) then {
            ...
            _group = [_markername,3,_snipers,_soldiers,"",_respawn] call SAR_fnc_AI_infantry;
...
for "_i" from (count _grps_sold) to ((_max_grps select 1) - 1) do
{
    if(_max_p_grp select 1 > 0) then {
        ...
        if(_chance < _probability) then {
            ...
            _group = [_markername,1,_snipers,_soldiers,"",_respawn] call SAR_fnc_AI_infantry;
            _grps_upd set [count _grps_upd,_group];
            // update AI monitor
...
for "_i" from (count _grps_surv) to ((_max_grps select 2) - 1) do
{
    if(_max_p_grp select 2 > 0) then {
        ...
        if(_chance < _probability) then {
            ...
            _group = [_markername,2,_snipers,_soldiers,"",_respawn] call SAR_fnc_AI_infantry;
            _grps_upd set [count _grps_upd,_group];
            // update AI monitor
