...
while {true} do {
	...
	if (_spawnRoll <= _spawnChance) then {
		...
		[_landingzone] joinSilent _aigroup;
		...
		_helipilot setVariable ["SAR_protect",true,true];
		...
		if (SAR_HC) then {
			{
				...
				if(_hcID select [0,2] isEqualTo 'HC')then {
					...
					if (_SAIS_HC) then {
						if (SAR_DEBUG) then {
							diag_log format ["Sarge AI System: Now moving heli crash group %1 to Headless Client %2",_aigroup,_hcID];
						};
					} else {
						if (SAR_DEBUG) then {
							diag_log format ["Sarge AI System: ERROR! Moving heli crash group %1 to Headless Client %2 has failed!",_aigroup,_hcID];
						};
					};
				};
			} forEach allPlayers;
